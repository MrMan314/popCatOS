...
#define HEIGHT  200
#define DEPTH   0x8
#define FBSIZE  WIDTH * HEIGHT * DEPTH

uint8 *fb = (uint8*) 0xA0000;
uint8 *bb = (uint8*) 0x11D000;
...
void write(char c, uint16 x, uint16 y, uint8 color) {
    uint8 *img = font[(uint16) c];
    for (uint16 yy = 0; yy < 8; yy++) {
        for (uint16 xx = 0; xx < 8; xx++) {
            if (img[yy] & (1 << xx)) {
                *(bb + 320 * (y + yy) + (x + xx)) = color;
            }
        }
    }
    memcpy(fb, bb, FBSIZE);
